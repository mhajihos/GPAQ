...
names(Res)[c(2,3,4,5,6,7)]=c(colnames(S)[which(substr(colnames(S),1,10)=="High")],"95%CI_High",
				colnames(S)[which(substr(colnames(S),1,10)=="Moderate")],"95%CI_Moderate",
				colnames(S)[which(substr(colnames(S),1,10)=="Low")],"95%CI_Low")
}
detach(Data_subset)
return(Res)
}
